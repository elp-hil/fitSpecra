		...
		try:
			...
			if self.sumDataVar.get():
				if np.any(self.guiData.summedFitParameters):
					...
					self.widthParameter.delete(0,tkinter.END)
					self.frequencyParameter.delete(0,tkinter.END)
					self.heightParameter.insert(0,self.guiData.summedFitParameters[0])
					self.widthParameter.insert(0,self.guiData.summedFitParameters[2])
					self.frequencyParameter.insert(0,self.guiData.summedFitParameters[1])
					...
					print(yData)
				self.subplot.plot(self.guiData.frequencies[lowerIndex:upperIndex], np.real(cmath.exp(1j*self.guiData.summedPhase)*  self.guiData.summedFftData[-self.guiData.frequencies.size+lowerIndex:-self.guiData.frequencies.size+upperIndex]))
			else:
				if self.guiData.fitParameters[self.scanNumber].any():
					...
					self.subplot.plot(self.guiData.frequencies[lowerIndex:upperIndex], yData)
					#self.subplot.plot(self.guiData.frequencies[lowerIndex:upperIndex])
